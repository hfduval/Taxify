...
public class Scooter extends Vehicle {
    ...
    public double calculateCost(IService service) {
        return super.calculateCost(service) * 0.75;
    }

    @Override
    public String toString() {
        if (this.getStatus() == VehicleStatus.FREE) {
            return "Scooter    " + this.getId() + " at " + this.getLocation() + " is free and ready to be booked";
        } else if (this.getStatus() == VehicleStatus.BOOKED) {
            return "Scooter    " + this.getId() + " at " + this.getLocation() + " is being picked up by user " +
